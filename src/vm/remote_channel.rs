...
mod macos {
    ...
    /// connection reads a length-prefixed JSON `RemoteMessage` frame, validates
    /// the destination against the allowlist, logs the message, and proxies the
    /// payload to the target remote Lobster endpoint.
    ///
    ...
    async fn handle_connection(
        ...
        match proxy_to_remote(&message).await {
            Ok(response) => {
                // Relay the response back to the VM.
    ...
    ///
    /// Pure function: produces a structured log entry and returns.
    ...
    async fn proxy_to_remote(message: &RemoteMessage) -> anyhow::Result<Vec<u8>> {
        ...
        // Serialise and send the payload as newline-delimited JSON.
